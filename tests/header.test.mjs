...
test("arrival offers Codex or manual starts and keeps the rough plan directly editable", () => {
  ...
  assert.equal(source.match(/<article class="starter-report-card/g)?.length, 4);
  for (const label of ["Dates", "Total budget", "Budget split"]) assert.match(source, new RegExp(`>${label}<`));
  ...
  assert.match(source, /data-overview-dialog="settings"/);
  ...
  assert.match(styles, /\.starter-report-strip \{ display:grid; grid-template-columns:repeat\(4,minmax\(0,1fr\)\)/);
