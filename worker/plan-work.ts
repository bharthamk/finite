...
export const handlePlanWorkRequest = async (request: Request, db: D1Database, files?: FiniteFilesBucket): Promise<Response | null> => {
  ...
  try {
    ...
    if (url.pathname === "/api/plan-work/attachments/upload") {
      ...
      const hash = await authSha256({ scopeId, operation: "upload", planId: guard.planId, revision: guard.revision, section: guard.section, contextId: guard.contextId, attachmentRole: role.role, fileName: file.name, size: file.size, type: file.type });
    ...
    if (attachmentProcessing) {
      ...
      const payload = { ok: true, code: "PLAN_ATTACHMENT_PROCESSING_UPDATED", attachment: toAttachment(updated, head.revision), ...(await listWork(db, scopeId, guard.planId, head.revision)), acceptedStateChanged: false as const };
      await db.batch([db.prepare("INSERT INTO plan_work_receipts (scope_id, idempotency_key, request_hash, receipt_json, created_at) VALUES (?, ?, ?, ?, ?)").bind(scopeId, guard.key, hash, JSON.stringify(payload), now)]);
      return respond(200, payload);
    }
    if (url.pathname === "/api/plan-work/attachments") {
      ...
      await db.batch([db.prepare("INSERT INTO plan_attachments (scope_id, attachment_id, plan_id, plan_revision, section, context_id, context_label, kind, label, note_text, link_url, object_key, file_name, content_type, size_bytes, source_surface, attachment_role, processing_status, processing_summary, derived_refs_json, processed_by, processed_at, status, created_at, updated_at, removed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, NULL, NULL, ?, ?, ?, NULL, '[]', NULL, NULL, 'active', ?, ?, NULL)").bind(scopeId, attachmentId, guard.planId, guard.revision, guard.section, guard.contextId, guard.contextLabel, attachment.kind, attachment.label, attachment.noteText, attachment.linkUrl, guard.source, role.role, processingStatus, now, now)]);
    ...
    if (attachmentRemove) {
      ...
      if (!existing) return respond(404, { ok: false, code: "PLAN_ATTACHMENT_NOT_FOUND", ...(await listWork(db, scopeId, guard.planId, head.revision)), acceptedStateChanged: false });
      await db.batch([db.prepare("UPDATE plan_attachments SET status = 'removed', removed_at = ?, updated_at = ? WHERE scope_id = ? AND attachment_id = ? AND plan_id = ?").bind(now, now, scopeId, targetId, guard.planId)]);
