...
const savePacket = async (db: D1Database, scopeId: string, body: JsonRecord): Promise<Response> => {
  ...
  if (existing && existing.source_order_id === source?.orderId && existing.source_order_version !== null && existing.source_order_version > Number(source.orderVersion)) return errorResponse(409, "CONSTRUCTION_PACKET_VERSION_REGRESSION", "A newer version of this same arrival-bound construction packet already exists.", { currentPacketId: existing.packet_id, currentOrderId: existing.source_order_id, currentOrderVersion: existing.source_order_version });
  if (existing && !existing.cleared_at && existing.source_order_id === source?.orderId && existing.source_order_version === source?.orderVersion && existing.kind === "draft" && packet.kind === "intake") return errorResponse(409, "CONSTRUCTION_PACKET_PHASE_REGRESSION", "A current compiled draft cannot be replaced by an earlier intake phase for the same order version.", { currentPacketId: existing.packet_id });
