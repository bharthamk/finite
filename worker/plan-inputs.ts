...
import { planInputKinds, planInputSections, validatePlanInput, type PlanInputKind, type PlanInputRecord, type PlanInputSection, type PlanInputSource } from "../src/plan-input.js";

type JsonRecord = Record<string, unknown>;
type PlanInputRow = { input_id: string; plan_id: string; plan_revision: number; kind: PlanInputKind; section: PlanInputSection; context_id: string | null; context_label: string | null; message: string; status: "open" | "handled"; source_surface: PlanInputSource; created_at: string; handled_at: string | null };
...
const rowToRecord = (row: PlanInputRow, currentRevision: number): PlanInputRecord => ({ inputId: row.input_id, planId: row.plan_id, planRevision: row.plan_revision, kind: row.kind, section: row.section, contextId: row.context_id, contextLabel: row.context_label, message: row.message, status: row.status, sourceSurface: row.source_surface, createdAt: row.created_at, handledAt: row.handled_at, baseCurrent: row.plan_revision === currentRevision });
const listInputs = async (db: D1Database, scopeId: string, planId: string, currentRevision: number): Promise<PlanInputRecord[]> => {
  const rows = await db.prepare("SELECT input_id, plan_id, plan_revision, kind, section, context_id, context_label, message, status, source_surface, created_at, handled_at FROM plan_inputs WHERE scope_id = ? AND plan_id = ? AND status = 'open' ORDER BY created_at DESC").bind(scopeId, planId).all<PlanInputRow>();
...
export const handlePlanInputRequest = async (request: Request, db: D1Database): Promise<Response | null> => {
  ...
  if (url.pathname !== "/api/plan-inputs" && !resolveMatch) return null;
  if (!["GET", "POST"].includes(request.method) || (resolveMatch && request.method !== "POST")) return response(405, { ok: false, code: "METHOD_NOT_ALLOWED", inputs: [], acceptedStateChanged: false });
  ...
  try {
    ...
    const operation = resolveMatch ? "resolve" : "add";
    const inputId = resolveMatch ? decodeURIComponent(resolveMatch[1]!) : "";
    if (resolveMatch && !safeId(inputId)) return response(422, { ok: false, code: "PLAN_INPUT_ID_INVALID", inputs: [], acceptedStateChanged: false });
    const requestHash = await authSha256({ scopeId, operation, planId, expectedRevision, sourceSurface, ...(resolveMatch ? { inputId } : { kind: body.kind, section: body.section, contextId: body.contextId ?? null, contextLabel: body.contextLabel ?? null, message: body.message }) });
    ...
    if (resolveMatch) {
      const existing = await db.prepare("SELECT input_id, plan_id, plan_revision, kind, section, context_id, context_label, message, status, source_surface, created_at, handled_at FROM plan_inputs WHERE scope_id = ? AND input_id = ? AND plan_id = ?").bind(scopeId, inputId, planId).first<PlanInputRow>();
    ...
    const validation = validatePlanInput({ kind: body.kind, section: body.section, contextId: body.contextId, contextLabel: body.contextLabel, message: body.message });
    ...
    await db.batch([
      db.prepare("INSERT INTO plan_inputs (scope_id, input_id, plan_id, plan_revision, kind, section, context_id, context_label, message, status, source_surface, created_at, handled_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, NULL)").bind(scopeId, newInputId, planId, expectedRevision, item.kind, item.section, item.contextId, item.contextLabel, item.message, sourceSurface, now),
      db.prepare("INSERT INTO plan_input_receipts (scope_id, idempotency_key, request_hash, receipt_json, created_at) VALUES (?, ?, ?, ?, ?)").bind(scopeId, idempotencyKey, requestHash, JSON.stringify(payload), now),
    ]);
