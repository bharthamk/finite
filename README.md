...
- A no-model starter-plan surface after brief review: the human can preview a lightweight family-appropriate sequence before acceptance, automatically open the same draft afterward, inspect the exact reviewed details and working choices, save additions or corrections directly, and see those changes waiting for reconciliation. Codex develops the draft when requested; it is no longer the gate to viewing or shaping the plan.
