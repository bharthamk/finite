...
export const createCodexHandoff = (context: CodexHandoffContext): CodexHandoff => {
  ...
  const prompt = [
    ...
    ...(demoPlayback ? [
      ...
      "Run four prepared chapters. Chapter one must begin on Finite's actual first page headed 'How do you want to begin?', before any route has been chosen and before the blank starting form is open. Call finite_guide_view for the arrival surface and top target with a concise explanation of the four ordinary routes and pauseForNext true. Do not click a route before the person clicks Next. When Finite releases that gate, use the visible route labelled 'Start fresh' exactly as a person would. Then call finite_guide_view on the arrival surface with pauseForNext false, use the selected visible browser surface to focus the first field labelled 'What do you want to plan?', and visibly type the complete synthetic Hobart brief into that field character by character. Do not use direct DOM assignment and do not call finite_create_arrival_order. Once the full brief is visibly present but still unsubmitted, call finite_guide_view on the starting point with a concise explanation and pauseForNext true. The person must experience both the true first page and the ordinary first form before anything is saved.",
      "When the person clicks Next at any gate, retry the intended finite_guide_view call with pauseForNext false until Finite releases it. Their Next click is the complete continuation signal: do not ask them to type 'proceed', send another message, or operate any other control. Only after the form gate releases, use the visible page control labelled 'Build my rough plan' to submit the first form; the real interface must create the saved starting point. Chapter two builds and shows the structured draft. Chapter three adds a synthetic rainy-day change and shows how the editable draft adapts. Chapter four shows the resulting plan summary and what remains editable. End chapters two, three, and four with finite_guide_view and pauseForNext true. Do not begin the next chapter until Finite releases the intended guide call after the visible Next click; keep the Codex turn alive and retry only at modest intervals while it reports that it is waiting.",
