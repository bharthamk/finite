...
export const createCodexHandoff = (context: CodexHandoffContext): CodexHandoff => {
  ...
  const prompt = demoPlayback && demoDepth === "spotlight" ? [
    "Take over this Finite plan as the Codex operator.",
    "",
    "Your first interaction with the person must be this exact question, with no browser action or Finite action before it: Where would you like to watch me run Finite: in a controlled browser window, or in the Codex built-in browser? Wait for their explicit answer.",
    `Use exactly one visible tab in the browser they choose and navigate it to ${siteEntryUrl}. Keep that same tab for the run. If it cannot be shown at the Finite Site origin, stop and tell the person; do not switch browser surfaces yourself.`,
    "",
    `Finite at ${siteOrigin} is the live plan surface. Run one synthetic Spotlight transaction on its real already-active 18-day Europe plan. The visible 'Demo mode · Local only' badge must be present before any write. Never use personal data or take any external action.`,
    "Discover that tab's page tools. If finite_enter_kitchen is not ready, call finite_webmcp_status, wait for WEBMCP_READY and refresh discovery. Then make this your first Finite call:",
    `finite_enter_kitchen(${JSON.stringify(toolInput)})`,
    "",
    "Follow its canonical nextAction. First open the planning toolset. Then call finite_guide_view on surface current and target plan_summary with pauseForNext false. Say: This plan is underway. Its flights and total budget are fixed, Paris matters, and at least A$500 of freedom must remain. Now reality changes.",
    "Invoke finite_record_change_event through finite_invoke with the canonical current revision and exactly: type = intent_change; title = Add three nights in Paris; costDeltaMinor = 66000; daysDelta = 3; minimumBufferMinor = 50000; evidenceRefs = [evidence_current]; entityChanges = [{ entityId: trip_days, field: days, delta: 3 }, { entityId: booked_segment_days, field: days, delta: 3 }]. Use delta, never value. This records pressure only and must not change accepted truth.",
    "Use its eventId to invoke finite_compare_options with generate true. Let Finite enumerate the legal move space. Then call finite_guide_view on surface current and target options with pauseForNext false. Say: Finite checked the legal combinations against the fixed flights, trip length, preferences and A$500 floor. These routes work differently. You choose what the plan becomes.",
    "Stop there. Ask the person to use a visible 'Use this option' control, review the exact effect, and choose 'Confirm and update plan' if they want it. Do not click either control, choose, confirm or create authority for them.",
    "After the person confirms, enter the kitchen again with the exact current plan identity. Continue only when canonical nextAction reports that human authority is present. Invoke finite_apply_approved_option with its exact candidateId, approvalId, revision and idempotency requirements.",
    "After apply succeeds, guide target receipt without a Next pause. Say: Your chosen route is now the accepted plan. Finite applied one revision-bound change and left a replay-safe receipt. Then read finite_get_effort_receipt and report calls to first useful action, failed calls, and completion.",
    "",
    "Before every state-changing call, obey operatorPacket.preMutationGate and use exact canonical identifiers. Ask the person for exactly one product decision: which valid route to use. This prompt carries no credentials or human authority.",
  ].join("\n") : [
    ...
    ...(demoPlayback ? demoDepth === "spotlight" ? [
      ...
      "First open the planning toolset, following the canonical nextAction returned by finite_enter_kitchen. Then call finite_guide_view on surface current and target plan_summary with pauseForNext false using this exact overlay message: This plan is already underway. The flights and total budget are fixed, Paris matters, and at least A$500 of freedom must remain. Now reality changes.",
