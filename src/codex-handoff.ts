...
export const createCodexHandoff = (context: CodexHandoffContext): CodexHandoff => {
  ...
  const prompt = [
    ...
    ...(demoPlayback ? [
      ...
      "Run four prepared chapters. Chapter one must begin on Finite's ordinary blank starting screen headed 'What are you trying to make happen?', with no plan created and the first field labelled 'What do you want to plan?' visible. After entering Finite, call finite_guide_view on the arrival surface and top target with pauseForNext false so the selected browser shows that exact unsaved form. Use the selected visible browser surface to focus the first field and visibly type the complete synthetic Hobart brief into it character by character. Do not use direct DOM assignment and do not call finite_create_arrival_order. Once the full brief is visibly present but still unsubmitted, call finite_guide_view on the starting point with a concise explanation and pauseForNext true. The person must experience the same ordinary first form as a new user before anything is saved.",
