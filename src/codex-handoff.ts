...
export const createCodexHandoff = (context: CodexHandoffContext): CodexHandoff => {
  ...
  const prompt = [
    ...
    ...(demoPlayback ? [
      ...
      `${demoDepth === "basics" ? "Run two prepared chapters" : demoDepth === "complete" ? "Run six prepared chapters" : "Run four prepared chapters"}. Chapter one must begin on Finite's ordinary blank starting screen headed 'What are you trying to make happen?', with no plan created and the first field labelled 'What do you want to plan?' visible. After entering Finite, call finite_guide_view on the arrival surface and top target with pauseForNext false so the selected browser shows that exact unsaved form and explains that this is the same starting point every new user sees. Then call finite_guide_view on target plan_ideas with pauseForNext false and use the normal visible interface to click the idea labelled 'Weekend trip'. Do not assign field values through the DOM.`,
      "Next call finite_guide_view on target planning_window with pauseForNext false. Focus the visible 'What do you want to plan?' field and visibly change the suggested sentence into this complete brief using normal keyboard input: Plan a three-night Hobart trip for two adults departing Sydney on 16 October 2026, with an AUD 2,400 total budget, an easy pace, MONA, excellent local food, and one flexible nature day. The person must see the template become their own plan inside the planning window.",
      "Next call finite_guide_view on target build_method with pauseForNext false and click the visible tab labelled 'Build it myself'. Call finite_guide_view on target manual_details with pauseForNext false, then visibly fill every structured field through the normal interface: What are you planning? = Plan a three-night Hobart trip for two adults from Sydney; When? = Depart Sydney 16 October 2026 and return after three nights; What is limited? = AUD 2,400 total budget and an easy pace; What must not change? = Include MONA, excellent local food, and keep one nature day flexible for weather; Useful references = Synthetic public demo only — no bookings or personal information. Do not call finite_create_arrival_order and do not submit either starting method yet.",
      "Once those structured details are visibly present but still unsubmitted, call finite_guide_view on target manual_details with a concise explanation and pauseForNext true. This is Chapter one's natural pause: the visible overlay must show Next and tell the person they may ask Codex questions. The person must experience the same ordinary first form as a new user before anything is saved.",
      "When the person clicks Next at any gate, retry the intended finite_guide_view call with pauseForNext false until Finite releases it. Their Next click is the complete continuation signal: do not ask them to type 'proceed', send another message, or operate any other control. Only after the form gate releases, use the visible page control labelled 'Open my workspace' to submit the active manual form; the real interface must create the saved starting point.",
      ...(demoDepth === "basics" ? [
        "Chapter two shows the resulting editable draft without touring every control. Use finite_guide_view on the plan_summary target to explain the dates, budget, main sections, open questions, and that the person can change anything themselves or continue with Codex. Pause with pauseForNext true. Do not alter the draft, approve it, start managing, create human authority, or enter any consequential flow. After that final Next gate releases, use finite_guide_view without a Next pause to say: Basics complete. You have seen a starting point become an editable plan. Ask me anything in Codex, or leave the demo when you are ready.",
      ] : [
        "Chapter two shows the resulting structured draft while it remains on the real editable workspace. Use finite_guide_view on the top and priority targets to explain the plan-at-a-glance summary, Calendar, dates, budget, transport, accommodation, commitments, and tasks, then pause on the priority target with pauseForNext true. Do not approve, start managing, compile away from, or otherwise leave this editable workspace.",
        "Chapter three adds a synthetic rainy-day change and shows how the editable draft adapts. After the Next gate releases, use finite_guide_view on the updates target to explain that reality can change without rebuilding the plan. Read the canonical arrival order and exact provisional Calendar record identity, then call finite_save_workspace_records to update only that operator-editable provisional record: Saturday becomes the indoor MONA and excellent-local-food day, while Sunday remains the flexible nature day if conditions improve. Do not call finite_append_arrival_input because this synthetic fact is operator-supplied demo material, not human input. Do not use the human 'Add to request' form. Keep dates, budget, and all non-weather constraints unchanged. Show the adapted Calendar with finite_guide_view on the priority target and pauseForNext true.",
        ...(demoDepth === "complete" ? [
          "Chapter four demonstrates a plan-specific workspace. After the Next gate releases, read the exact canonical arrival identity, open the planning toolset when required, and use finite_save_workspace_module to add a provisional checklist section named Weather watch with stable moduleId custom_weather_watch and editable title, check time, forecast, decision, and notes fields. Then use finite_save_workspace_records to add two coherent synthetic checklist items: check the Saturday rain outlook the evening before, and make the final nature-day call by 7:00 am. Keep the new section outside accepted truth. Show the exact custom_weather_watch section with finite_guide_view on the priority target and pauseForNext true.",
          "Chapter five demonstrates comparison without pretending a suggestion is part of the plan. After the Next gate releases, read the exact existing accommodation section and provisional parent record identity. Use finite_save_workspace_option to add two clearly synthetic, unresearched stay ideas using only fields allowed by the returned section schema; do not invent availability, booking status, prices, ratings, or source links. Explain that options remain outside plan maths and commitments until the person chooses one. Show that exact accommodation section with finite_guide_view on the priority target and pauseForNext true.",
          "Chapter six shows the resulting plan summary and everything that remains editable. After the Next gate releases, call finite_guide_view on the plan_summary target, explain the dates, total budget, rainy-day adaptation, custom Weather watch section, comparison ideas, remaining provisional work, and open questions, then pauseForNext true. Do not approve the plan, start managing, create human authority, or enter any consequential flow. After that final Next gate releases, use finite_guide_view without a Next pause to say: Full tour complete. Ask me anything about what you saw in Codex, or leave the demo when you are ready.",
        ] : [
          "Chapter four shows the resulting plan summary and what remains editable. After the Next gate releases, call finite_guide_view on the plan_summary target, explain the dates, total budget, remaining provisional work, open questions, and that every workspace section can still be edited, then pauseForNext true. Do not approve the plan, start managing, create human authority, or enter any consequential flow. After that final Next gate releases, use finite_guide_view without a Next pause to say: Demo complete. Ask me anything about what you saw in Codex, or leave the demo when you are ready.",
        ]),
      ]),
      "Do not begin any later chapter until Finite releases the intended guide call after the visible Next click; keep the Codex turn alive and retry only at modest intervals while it reports that it is waiting.",
