  ...
  const prompt = [
    ...
    ...(demoPlayback ? [
      ...
      ...(demoDepth === "basics" ? [
        ...
        "Chapter four adds a synthetic rainy-day change and shows how the editable draft adapts. After the Next gate releases, call finite_guide_view on the updates target with pauseForNext true using this exact overlay message: Plans change in real life. Add a new fact here and Finite can adapt the working plan without making you start again. Do not add anything yet. After that gate releases, read the canonical arrival order and exact provisional Calendar record identity, then call finite_save_workspace_records to update only that operator-editable provisional record: Saturday becomes the indoor MONA and excellent-local-food day, while Sunday remains the flexible nature day if conditions improve. Do not call finite_append_arrival_input because this synthetic fact is operator-supplied demo material, not human input. Do not use the human 'Add to request' form. Keep dates, budget, and all non-weather constraints unchanged. Show the adapted Calendar with finite_guide_view on the priority target and pauseForNext true.",
        ...(demoDepth === "complete" ? [
