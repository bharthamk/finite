...
const money = (minor: number, currency: string): string => new Intl.NumberFormat("en-AU", { style: "currency", currency, currencyDisplay: "code", maximumFractionDigits: 0 }).format(minor / 100);
const measure = (value: string | number, format: string, currency: string): string => format === "money" && typeof value === "number" ? money(value, currency) : format === "days" && typeof value === "number" ? `${value} day${value === 1 ? "" : "s"}` : format === "percent" && typeof value === "number" ? `${value}%` : String(value);
const brand = (): string => `<a class="brand" href="#main" aria-label="Finite home"><img src="/finite-wordmark.png" width="98" height="30" alt=""></a>`;

export const projectionMarkup = (projection: PublicPlanProjection): string => {
  const { plan } = projection;
  const currency = plan.currencyCode ?? "AUD";
  ...
  ${plan.allocation ? `<section class="published-plan__allocation"><h2>The finite total</h2><dl><div><dt>Total</dt><dd>${money(plan.allocation.totalBudgetMinor, currency)}</dd></div><div><dt>Spent</dt><dd>${money(plan.allocation.spentMinor, currency)}</dd></div><div><dt>Committed</dt><dd>${money(plan.allocation.committedMinor, currency)}</dd></div><div><dt>Forecast</dt><dd>${money(plan.allocation.forecastMinor, currency)}</dd></div><div class="is-buffer"><dt>Remaining</dt><dd>${money(plan.allocation.bufferMinor, currency)}</dd></div></dl></section>` : ""}
  ${plan.measures?.length ? `<section class="published-plan__measures"><h2>Key measures</h2><dl>${plan.measures.map((item) => `<div><dt>${escapeHtml(item.label)}</dt><dd>${escapeHtml(measure(item.value, item.format, currency))}</dd></div>`).join("")}</dl></section>` : ""}
