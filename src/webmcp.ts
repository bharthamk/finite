...
import { HttpPlanWorkRepository, type AttachmentProcessingStatus, type PlanAttachment, type PlanWorkRepository, type PlanWorkResult } from "./plan-work.js";
...
const enterKitchen = async (runtime: FinitePlanRuntime, arrival: ArrivalRepository, planInputs: PlanInputRepository, planWork: PlanWorkRepository, planLearning: PlanLearningRepository, input: Record<string, unknown>, context: { signal?: AbortSignal } = {}): Promise<ToolResult> => {
...
const getChefMenu = async (runtime: FinitePlanRuntime, arrival: ArrivalRepository, planInputs: PlanInputRepository, planWork: PlanWorkRepository, planLearning: PlanLearningRepository, input: Record<string, unknown>, context: { signal?: AbortSignal } = {}): Promise<ToolResult> => {
...
export const finiteGuideTargets = ["top", "plan_ideas", "planning_window", "build_method", "manual_details", "starting_point", "status", "question", "priority", "open_questions", "review", "interpretation", "updates", "plan_summary", "section_headers", "budget_editor", "stages", "options", "approval", "receipt"] as const;
...
export class FinitePlanWebMCPAdapter {
  ...
  constructor(private readonly host: ModelContextHost, private readonly runtime: FinitePlanRuntime, private readonly observer?: WebMCPToolObserver, private readonly arrival: ArrivalRepository = new HttpArrivalRepository(), private readonly entryAlreadyRegistered = false, private readonly reset: KitchenResetRepository = new HttpKitchenResetRepository(), private readonly onKitchenReset: (result: KitchenResetResult) => Promise<void> = async () => {}, private readonly themes: ThemeRepository = new HttpThemeRepository(), private readonly onThemeChanged: (result: ThemeResult) => Promise<void> = async () => {}, private readonly skins: SkinRepository = new HttpSkinRepository(), private readonly onSkinChanged: (result: SkinResult) => Promise<void> = async () => {}, private readonly planInputs: PlanInputRepository = new HttpPlanInputRepository(), private readonly onPlanInputsChanged: (result: PlanInputResult) => Promise<void> = async () => {}, private readonly planWork: PlanWorkRepository = new HttpPlanWorkRepository(), private readonly onPlanWorkChanged: (result: PlanWorkResult) => Promise<void> = async () => {}, private readonly planLearning: PlanLearningRepository = new HttpPlanLearningRepository(), private readonly onPlanLearningChanged: (result: PlanLearningResult) => Promise<void> = async () => {}) {}
