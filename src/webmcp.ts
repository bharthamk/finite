...
export const humanOnlyActions = Object.freeze(["humanApprove", "humanConfirmActualCorrection", "humanConfirmPreferenceChange", "humanConfirmPlanDraft"]);
...
const coreDefinitions = (runtime: FinitePlanRuntime, onProfileChanged: () => Promise<void>): WebMCPToolDefinition[] => [
  ...
  define({ name: "finite_activate_confirmed_plan", title: "Activate a human-confirmed plan", description: "Activate only the exact compiled plan draft confirmed by a human, bound to the active plan and revision.", inputSchema: objectSchema({ draftId: string, confirmationId: string, expectedPlanId: string, expectedRevision: revision, idempotencyKey }, ["draftId", "confirmationId", "expectedPlanId", "expectedRevision", "idempotencyKey"]), execute: async (input) => { const result = await runtime.activateConfirmedPlanDraft(input as never); if (result.ok && result.code === "PLAN_ACTIVATED") await onProfileChanged(); return result; } }),
