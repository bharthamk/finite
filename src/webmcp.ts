...
const coreDefinitions = (runtime: FinitePlanRuntime, onProfileChanged: () => Promise<void>): WebMCPToolDefinition[] => [
  ...
  define({ name: "finite_compare_options", title: "Compare normalized options", description: "Generate or compare distinct options against one event and revision with exact measures and preference scores.", readOnly: true, inputSchema: objectSchema({ eventId: string, generate: { type: "boolean" } }, ["eventId"]), execute: (input) => runtime.kernel.compareOptions(input as never) }),
