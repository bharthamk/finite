...
import { HttpPlanInputRepository, type PlanInputKind, type PlanInputRepository, type PlanInputResult, type PlanInputSection } from "./plan-input.js";
...
const routeRefreshToolNames = new Set([
  ...
  "finite_add_plan_input", "finite_resolve_plan_input",
...
const coreDefinitions = (runtime: FinitePlanRuntime, onProfileChanged: () => Promise<void>, arrival: ArrivalRepository, reset: KitchenResetRepository, onKitchenReset: (result: KitchenResetResult) => Promise<void>, themes: ThemeRepository, onThemeChanged: (result: ThemeResult) => Promise<void>, skins: SkinRepository, onSkinChanged: (result: SkinResult) => Promise<void>, planInputs: PlanInputRepository, onPlanInputsChanged: (result: PlanInputResult) => Promise<void>, guideView: FiniteGuideViewHandler): WebMCPToolDefinition[] => [
  ...
  define({ name: "finite_list_plan_inputs", title: "List open plan decisions and updates", description: "Read the person's open decisions, updates, and questions for the active plan, including the section or exact stage they attached each item to. These are human-originated working inputs, not accepted plan mutations or approval authority.", readOnly: true, execute: (_input, context) => planInputs.list({ planId: runtime.kernel.profile.planId }, context) }),
  define({ name: "finite_add_plan_input", title: "Add a decision or update to the plan", description: "Add one person-requested decision, update, or question to the active plan's working queue, either generally or against a named timeline, money, or boundaries area. This does not itself rewrite accepted plan truth or grant approval.", inputSchema: objectSchema({ planId: string, expectedRevision: revision, kind: { type: "string", enum: ["decision", "update", "question"] }, section: { type: "string", enum: ["general", "timeline", "money", "boundaries"] }, contextId: { type: ["string", "null"], maxLength: 100 }, contextLabel: { type: ["string", "null"], maxLength: 120 }, message: { type: "string", minLength: 1, maxLength: 2000 }, idempotencyKey }, ["planId", "expectedRevision", "kind", "section", "message", "idempotencyKey"]), execute: async (input, context) => { const result = await planInputs.add({ planId: String(input.planId), expectedRevision: Number(input.expectedRevision), kind: input.kind as PlanInputKind, section: input.section as PlanInputSection, contextId: input.contextId ? String(input.contextId) : null, contextLabel: input.contextLabel ? String(input.contextLabel) : null, message: String(input.message), idempotencyKey: String(input.idempotencyKey), sourceSurface: "codex" }, context); if (result.ok) await onPlanInputsChanged(result); return result; } }),
...
export class FinitePlanWebMCPAdapter {
  ...
  constructor(private readonly host: ModelContextHost, private readonly runtime: FinitePlanRuntime, private readonly observer?: WebMCPToolObserver, private readonly arrival: ArrivalRepository = new HttpArrivalRepository(), private readonly entryAlreadyRegistered = false, private readonly reset: KitchenResetRepository = new HttpKitchenResetRepository(), private readonly onKitchenReset: (result: KitchenResetResult) => Promise<void> = async () => {}, private readonly themes: ThemeRepository = new HttpThemeRepository(), private readonly onThemeChanged: (result: ThemeResult) => Promise<void> = async () => {}, private readonly skins: SkinRepository = new HttpSkinRepository(), private readonly onSkinChanged: (result: SkinResult) => Promise<void> = async () => {}, private readonly planInputs: PlanInputRepository = new HttpPlanInputRepository(), private readonly onPlanInputsChanged: (result: PlanInputResult) => Promise<void> = async () => {}) {}
  ...
  async register(): Promise<string[]> {
    ...
    this.executableTools = [...coreDefinitions(this.runtime, () => this.refreshContextualTools(), this.arrival, this.reset, this.onKitchenReset, this.themes, this.onThemeChanged, this.skins, this.onSkinChanged, this.planInputs, this.onPlanInputsChanged, (request) => this.guideView(request)), openToolset];
