...
const coreDefinitions = (runtime: FinitePlanRuntime, onProfileChanged: () => Promise<void>, arrival: ArrivalRepository, reset: KitchenResetRepository, onKitchenReset: (result: KitchenResetResult) => Promise<void>, themes: ThemeRepository, onThemeChanged: (result: ThemeResult) => Promise<void>, skins: SkinRepository, onSkinChanged: (result: SkinResult) => Promise<void>, planInputs: PlanInputRepository, onPlanInputsChanged: (result: PlanInputResult) => Promise<void>, planWork: HttpPlanWorkRepository, onPlanWorkChanged: (result: PlanWorkResult) => Promise<void>, guideView: FiniteGuideViewHandler): WebMCPToolDefinition[] => [
  ...
  define({ name: "finite_save_workspace_option", title: "Save a suggested option in one plan area", description: "Add, update, or remove one Codex-suggested option in the editable rough plan. Options remain visibly non-authoritative and do not affect plan dates, counts, relationships, or money until the human chooses Add to plan.", inputSchema: objectSchema({ orderId: string, expectedVersion: revision, operation: { type: "string", enum: ["add", "update", "delete"] }, moduleId: string, optionId: { type: "string", minLength: 1, maxLength: 120 }, label: { type: "string", maxLength: 200 }, fields: { type: "object", maxProperties: 40, additionalProperties: { type: ["string", "boolean"] } } }, ["orderId", "expectedVersion", "operation", "moduleId", "optionId"]), execute: (input, context) => arrival.saveWorkspaceOption({ orderId: String(input.orderId), expectedVersion: Number(input.expectedVersion), operation: input.operation as "add" | "update" | "delete", moduleId: String(input.moduleId), optionId: String(input.optionId), ...(input.label !== undefined ? { label: String(input.label) } : {}), ...(input.fields && typeof input.fields === "object" && !Array.isArray(input.fields) ? { fields: input.fields as Record<string, string | boolean> } : {}) }, context) }),
