...
export const buildChefMenu = (kernel: FinitePlanKernel, route: KitchenRoute): { menuVersion: "finite-chef-menu.v1"; basis: Record<string, unknown>; items: ChefMenuItem[] } => {
  ...
  if (kernel.pendingGroupDecision && kernel.groupDecisionConfirmation) {
    const change = kernel.pendingGroupDecision;
    items = [{ menuItemId: `group_apply_${change.groupDecisionId}`, rank: 1, kind: "operator_action", title: "Record the group decision", offer: change.resolvedOutcome, whyNow: "The human confirmed the exact named positions, conflicts, protocol, and outcome.", status: "ready", viability: "constraint_validated", nextTool: "finite_apply_confirmed_group_decision", knownArgs: { groupDecisionId: change.groupDecisionId, confirmationId: kernel.groupDecisionConfirmation.confirmationId, expectedRevision: kernel.revision }, missingInputs: [{ argument: "idempotencyKey", source: "derived", reason: "Codex must supply one stable retry identity." }], tradeoffs: change.unresolvedConflicts, evidence: { status: "not_required", refs: [] } }];
  } else if (kernel.pendingGroupDecision) {
    const change = kernel.pendingGroupDecision;
    items = [{ menuItemId: `group_review_${change.groupDecisionId}`, rank: 1, kind: "human_decision", title: "Confirm the group outcome?", offer: change.resolvedOutcome, whyNow: "Named positions and remaining disagreement are preserved, but no human authority exists.", status: "human_choice_required", viability: "constraint_validated", nextTool: null, knownArgs: { groupDecisionId: change.groupDecisionId, expectedRevision: kernel.revision }, missingInputs: [{ argument: "human_decision", source: "human", reason: "Codex cannot choose a decision protocol for people.", question: "Does this accurately preserve everyone's position and the outcome you want recorded?" }], tradeoffs: change.unresolvedConflicts, evidence: { status: "not_required", refs: [] } }];
  } else if (kernel.pendingExternalAction && kernel.externalActionConfirmation) {
    const change = kernel.pendingExternalAction;
    items = [{ menuItemId: `external_apply_${change.externalActionChangeId}`, rank: 1, kind: "operator_action", title: `Record ${change.after} status`, offer: `Append ${change.label} as ${change.after}; Finite does not claim it performed the action.`, whyNow: "The exact evidence-bound status has human confirmation.", status: "ready", viability: "constraint_validated", nextTool: "finite_apply_confirmed_external_action", knownArgs: { externalActionChangeId: change.externalActionChangeId, confirmationId: kernel.externalActionConfirmation.confirmationId, expectedRevision: kernel.revision }, missingInputs: [{ argument: "idempotencyKey", source: "derived", reason: "Codex must supply one stable retry identity." }], tradeoffs: [change.reason], evidence: { status: change.evidenceRef ? "available" : "not_required", refs: change.evidenceRef ? [change.evidenceRef] : [] } }];
  } else if (kernel.pendingExternalAction) {
    const change = kernel.pendingExternalAction;
    items = [{ menuItemId: `external_review_${change.externalActionChangeId}`, rank: 1, kind: "human_decision", title: `Is ${change.label} really ${change.after}?`, offer: `Review the evidence and confirm the real-world status. No external action is performed here.`, whyNow: "The status is staged but cannot become canonical from a quote, inference, or silence.", status: "human_choice_required", viability: "constraint_validated", nextTool: null, knownArgs: { externalActionChangeId: change.externalActionChangeId, expectedRevision: kernel.revision }, missingInputs: [{ argument: "human_decision", source: "human", reason: "Only the human can attest the real-world state.", question: `Should Finite record ${change.label} as ${change.after}?` }], tradeoffs: [change.reason], evidence: { status: change.evidenceRef ? "available" : "not_required", refs: change.evidenceRef ? [change.evidenceRef] : [] } }];
  } else if (kernel.pendingLifecycleChange && kernel.lifecycleConfirmation) {
